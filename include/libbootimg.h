...
/**
 * Enriches the given bootimg structure with information about a possibly
 * contained FDT (kernel or DTB). In order to generate this information, the
 * corresponding blobs must be loaded.
 * @param b pointer to struct bootimg
 * @return zero if successful, negative value from libbootimg_error if failed.
 */
