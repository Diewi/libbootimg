...
#define CAST_INT_BYTE_PTR(i) (uint8_t*)(int32_t*)(uintptr_t)(i)
...
static uint8_t* libbootimg_find_fdt(uint8_t *start_addr, uint32_t length)
{
    ...
    dtb_appended = CAST_INT_BYTE_PTR(memmem(start_addr, length, fdt_magic,
            sizeof(fdt_magic)));
...
int libbootimg_get_fdt_info(struct bootimg *b)
{
    ...
    if (dtb_blob->size != NULL)
    {
        ...
        if (fdt_addr != NULL)
        {
            ...
            // Update the FDT size in the loaded blob, such that the size is
            // correct when being dumped.
            fdt_info.fdt_hdr->totalsize = fdt_info.fdt_size;
    ...
    if (kernel_blob->size == NULL)
    {
        LOG_DBG("Cannot search for appended FDT without kernel blob.\n");
        return -1;
    }
    ...
    if (fdt_addr != NULL)
    {
        ...
        // We require endianess conversion for ARM targets.
...
//    LOG_DBG("FDT_info: %x\n", b->fdt_info);
    LOG_DBG("Addr FDT: %x\n", b->fdt_info.fdt_hdr);
//    LOG_DBG("Addr FDT: %x\n", *(b->fdt_info->fdt_hdr));
    //int relative_addr = fdt_info->is_fdt_appended ? (fdt_info->fdt_hdr - (b->blobs[LIBBOOTIMG_BLOB_KERNEL].data)) : (fdt_info->fdt_hdr - (b->blobs[LIBBOOTIMG_BLOB_DTB].data))
    //printf("Relative addr: %x", relative_addr);
    ...
    if (fwrite(fdt_info->fdt_hdr, fdt_info->fdt_size, 1 ,fd) != 1)
    {
        res = LIBBOOTIMG_ERROR_IO;
    }
...
int libbootimg_load_fdt(struct bootimg *b, const char *src)
{
    ...
    if (libbootimg_find_fdt(fdt_new_data, 4 * sizeof(uint8_t)) == NULL)
    {
        LOG_DBG("The given file does not start with the FDT magic. Doing nothing.\n");
